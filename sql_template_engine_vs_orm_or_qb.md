...
8 | Поиск медленных запросов для оптимизации | Быстрее. SQL запрос можно найти в исходном коде по совпадению фрагментов | Медленнее. Запрос генерируется и найти это место в коде трудно, если ORM или QB не записывает в комментарии перед запросом указатель
