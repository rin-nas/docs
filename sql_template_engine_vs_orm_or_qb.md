...
8 | Поиск медленных запросов для оптимизации | Лучше. SQL запрос можно найти в исходном коде по совпадению его части | Хуже. Запрос генерируется и найти это место в коде трудно, если ORM или QB не записывает в комментарии перед запросом указатель
...
## Выводы
ORM или QB могут применяться в начале пути, когда важна скорость разработки: 
для быстрого построения оценочных прототипов и MVP. 
Для критической к скорости и надёжности работы функциональности нужен SQL шаблонизатор.
